...
FractalError DrawMandelbrot0(const EnvironmentInfo *const env_info)
{
    ...
    for (int y = lu_window_corner.y; y < rd_window_corner.y; y++)
    {
        for (int x = lu_window_corner.x; x < rd_window_corner.x; x++)
        {
            ...
            ON_GRAPH_MODE(
            sf::Color pixel_color = SetPixelColor(n, max_calc_iterations_num, DARK_TURQUOISE); // sf::Color::White;
...
FractalError DrawMandelbrot_cycles(const EnvironmentInfo *const env_info)
{
    ...
    for (size_t pixel_num_y = 0; pixel_num_y < window_heigh; pixel_num_y++, y0 += dy)
    {
        ...
        for (size_t pixel_num_x = 0; pixel_num_x < window_width; pixel_num_x += 4, x0 += dx * 4)
        {
            ...
            volatile size_t iteration_num_pckd [4] = {0, 0, 0, 0};
            ...
            for (size_t i = 0; i < 4; i++)
            {
                ...
                {
                    ON_GRAPH_MODE(
                    sf::Color pixel_color = SetPixelColor(iteration_num_pckd[i], max_calc_iterations_num, DARK_TURQUOISE); // sf::Color::White;
...
FractalError DrawMandelbrot_intrinsics(const EnvironmentInfo *const env_info)
{
    ...
    for (size_t pixel_num_y = 0; pixel_num_y < window_heigh; pixel_num_y++, y0 += dy)
    {
        ...
        for (size_t pixel_num_x = 0; pixel_num_x < window_width; pixel_num_x += 4, x0 += dx * 4)
        {
            ...
            volatile __m256d iteration_num_pd = _mm256_set1_pd(0);
            ...
            for (size_t i = 0; i < 4; i++)
            {
                ...
                {
                    ON_GRAPH_MODE(
                    sf::Color pixel_color = SetPixelColor(iteration_num_pd[i], max_calc_iterations_num, DARK_TURQUOISE);
...
FractalError DrawMandelbrot_deployment(const EnvironmentInfo *const env_info)
{
    ...
    for (size_t pixel_num_y = 0; pixel_num_y < window_heigh; pixel_num_y++, y0 += dy)
    {
        ...
        for (size_t pixel_num_x = 0; pixel_num_x < window_width; pixel_num_x += 4 * k, x0 += dx * 4 * k)
        {
            ...
            SET1_UNFOLD_8(volatile __m256d iteration_num_pd, 0);
...
{    
    volatile uint8_t color_part = (double) iterations_num / (double) MAX_SEQUENCE_N * 255;
    ...
    default:
        fprintf(stderr, "Unknown color in SetPixelColor()\n");
        return sf::Color::Red;
    }
}
