...
inline uint64_t rdtsc()
{
    uint32_t lo, hi;
    asm volatile (
        "rdtsc"          // Ассемблерная инструкция RDTSC
        : "=a" (lo), "=d" (hi)  // EAX → lo, EDX → hi
    );
    return ((uint64_t)hi << 32) | lo;  // Объединение 32-битных частей в 64-битное число
}
...
void TestBenchmark()
{
    ...
    fprintf(dest_file, "| border radius        | %.2lf   |\n\n", sqrtf(env_info.border_radius_sq)            );
